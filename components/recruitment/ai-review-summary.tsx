...
export function AIReviewSummary({ candidate }: { candidate: CandidateDetail }) {
  ...
  const band =
    score >= 85
      ...
      : score >= 70
        ? { label: "Possible match", tone: "text-amber-600", bar: "bg-amber-500" }
    ...
    <Card className="card-pad">
      ...
      <div className="mt-4 grid gap-3 sm:grid-cols-2">
        ...
        <div className="rounded-xl border border-emerald-100 bg-emerald-50/40 p-3">
          ...
          {matched.length || parsedSkills.length ? (
            <div className="mt-2 flex flex-wrap gap-1">
              {matched.map((s) => (
                <span key={s} className="rounded-full bg-emerald-100 px-2 py-0.5 text-[11px] font-medium text-emerald-800">
                  {s}
                </span>
              ))}
              {parsedSkills
                ...
                .map((s) => (
                  <span key={s} className="rounded-full bg-white px-2 py-0.5 text-[11px] text-emerald-700 ring-1 ring-emerald-200">
                    {s}
                  </span>
                ))}
            </div>
        ...
        <div className="rounded-xl border border-amber-100 bg-amber-50/40 p-3">
          ...
          {missing.length ? (
            <div className="mt-2 flex flex-wrap gap-1">
              {missing.map((g) => (
                <span key={g} className="rounded-full bg-amber-100 px-2 py-0.5 text-[11px] font-medium text-amber-800">
                  {g}
                </span>
              ))}
            </div>
      ...
      <div className="mt-3 rounded-xl border border-line p-3">
        <p className="flex items-center gap-1.5 text-[11px] font-semibold uppercase tracking-wide text-ink-faint">
          <Crosshair className="h-3.5 w-3.5 text-brand-500" /> Suggested focus for interview
        </p>
